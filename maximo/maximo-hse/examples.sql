...
GROUP BY permittype, permit_type_description
...
SELECT permitnum, siteid, permittype, enddate, datediff(DAY, enddate, current_timestamp()) AS days_overdue
FROM {{catalog}}.{{gold_schema}}.v_open_permits
...
-- IMPORTANT: hours_worked must be sourced from HR.
SELECT
...
SELECT
    moc_id, moc_reason, action_wonum, action_status, action_due_date,
...
SELECT
    incidentid, reportdate, incidentcategory, severity,
