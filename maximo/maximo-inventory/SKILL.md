...
3. **Filter `MATUSETRANS` by `ISSUETYPE` for "consumption."** The table logs `ISSUE`, `RETURN`, `TRANSFER`, `ADJUSTMENT`. Use `ISSUETYPE IN ('ISSUE','RETURN')` and net them; including `TRANSFER` double-counts (a transfer moves stock between storerooms, it doesn't consume it).
