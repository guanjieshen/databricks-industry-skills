...
-- =============================================================================
-- Trusted Assets. Bind Databricks SQL parameters at registration time:
--   :catalog        → customer UC catalog (e.g. eam)
--   :silver_schema  → Silver schema with the MBO tables
--   :metrics_schema → schema that holds these governed metric functions
--   :principal      → grant target (a group preferred, e.g. genie-users)
-- Register once via maximo-setup (preview-then-apply); do not run from the skill.
-- These functions subtract reservations (CURBAL - RESERVEDQTY) for "available".
-- STATUS literals (e.g. 'ACTIVE') are correct for stock deployments; resolve via
-- SYNONYMDOMAIN (gotcha 9) before registering if the customer added synonyms.
-- =============================================================================
